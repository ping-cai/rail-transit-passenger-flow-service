...
import costcompute.{CostCompose, Section, SectionTravelGraph}
...
class DynamicLogitDistribute(pathDynamicCost: java.util.Map[Path, Double],
                             minPathCost: (Path, Double),
                             private var distributionResult: DistributionResult,
                             sectionTravelGraph: SectionTravelGraph,
                             sectionWithDirectionMap: java.util.Map[Section, String]) extends LogitDistribute with AdjustParam[Double] {
  ...
  def this(pathDynamicCost: java.util.Map[Path, Double],
           ...
           timeInterval: Int) {
    this(pathDynamicCost, minPathCost, distributionResult, sectionTravelGraph, sectionWithDirectionMap)
...
object DynamicLogitDistribute {
  def main(args: Array[String]): Unit = {
    var startTime = "2018-01-01 15:15:58"
    startTime = DateExtendUtil.timeAdditionSecond(startTime, 1000)
    println(startTime)
  }
}